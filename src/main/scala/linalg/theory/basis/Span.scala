...
trait Span[V, W, F] {
     ...
     def span(vset: W)(implicit s: SetVecLike[W, F]): W  = s.rowReducedEchelon(vset)

     // If the system is consistent then it spans the space R#, where # = numrows.
     // Example 3.10 - singh book
     // page 198 = howard book
     //TODO
     def doesSetSpanTheSpace(vset: W)(implicit s: SetVecLike[W, F]): Boolean
     ...
     //page 197 howard
     def doesSetSpanTheVector(vset: W, v: V)(implicit vLike: VectorLike[V, F],
                                             s: SetVecLike[W, F]): Boolean
     //step 1 - set up augmented matrix with W | V and solve - check if consistent

     def isSpannedBy(v: V, vset: W)(implicit vLike: VectorLike[V, F],
                                    s: SetVecLike[W, F]): Boolean = doesSetSpanTheVector(vset, v)
     ...
     def getSpanningCoefficients(vset: W, v: V)(implicit s: SetVecLike[W, F]): Option[List[F]]
}
