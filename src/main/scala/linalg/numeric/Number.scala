...
object Number {
     ...
     implicit class NumberOps[/*R,*/ N : Number](current: N) extends Ordered[N]  {
          ...
          //def +(other: R): N = r.plusO(current, other)
          ...
          /*override*/ def compare(other: N): Int = n.minus(current, other).toDouble.toInt
