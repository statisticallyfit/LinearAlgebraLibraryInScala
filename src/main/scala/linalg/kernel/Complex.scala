...
object Complex {
     ...
     def powerDeMoivre[R](z: Complex[R], n:R)(implicit gen: RealNumber[R]): Complex[R] ={
          val theta: R = angle(z)
     ...
     def nthRootComplex[R](z: Complex[R], n: R)(implicit gen: RealNumber[R],
          ...
          val theNRoots: List[R] = List.tabulate[R](n.toInt)(k =>
               (theta + gen.two * gen.from(k) * trig.PI) / n)

          (modulus.nRoot(n), theNRoots)
     }

     def nthRootsOfUnity[R](z: Complex[R], n: R)(implicit gen: RealNumber[R], trig: Trig[R]): List[R] = {
          List.tabulate[R](n.toInt)(k => (gen.two * gen.from(k) * trig.PI) / n)
     }
